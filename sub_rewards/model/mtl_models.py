...
from model.embedding import GloveEmbedding, ElmoEmbedding, BertEmbedding
...
class MTL_Model4(nn.Module):
    def __init__(self, args, device, collect_da_predictions=True):
        ...
        self.ff_u = nn.Linear(2*self.hidden_size_d, self.num_dialogacts)
        self.ff_d = nn.Linear(2*self.hidden_size_d, 1)
        nn.init.normal_(self.ff_d.weight, mean=0, std=1)
        nn.init.normal_(self.ff_u.weight, mean=0, std=1)
...
class MTL_Elmo1(nn.Module):
    def __init__(self, args, device, collect_da_predictions=True):
        super(MTL_Elmo1, self).__init__()
    ...
    def forward(self, x_dialogues, x_acts, lengths):
        ...
        loss_da = torch.zeros(ten_acts.size(0)).to(self.device)
        H = self.attn_u(x.view(old_size[0]*old_size[1], old_size[2], old_size[3]))
...
class MTL_Bert(nn.Module):
    def __init__(self, args, device, collect_da_predictions=True):
        super(MTL_Bert, self).__init__()
        self.input_size = 768
        self.hidden_size_u = args.lstm_sent_size
        self.hidden_size_d = args.lstm_utt_size
        self.num_layers = args.lstm_layers
        self.num_dialogacts = args.num_classes
        self.device = device
        self.emb = BertEmbedding(args, device)
        self.only_da = True if args.loss == 'da' else False
    ...
    def forward(self, x_dialogues, x_acts, lengths):
        ...
        H = self.emb(x_dialogues)
        old_size = (H.size(0), H.size(1), H.size(2))
        # ten_sents = x.view(old_size[0]*old_size[1], old_size[2], old_size[3]) 
        ten_acts = x_acts.view(old_size[0]*old_size[1]) 
        ...
        # H = H.unsqueeze(0)
        if not self.only_da:
            h0 = torch.zeros(self.num_layers*2, H.size(0), self.hidden_size_d).to(self.device)# 2 for bidirection 
            c0 = torch.zeros(self.num_layers*2, H.size(0), self.hidden_size_d).to(self.device)
            H = pack_padded_sequence(H, d_lengths, batch_first=True, enforce_sorted=False)
